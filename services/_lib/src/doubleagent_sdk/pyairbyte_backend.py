...
_INSTALL_MSG = (
    ...
    "Or use --backend docker to use the Docker-based adapter."
)
...
def image_to_connector_name(image: str) -> str:
    ...
    """
    # Strip registry prefix (e.g., "airbyte/")
    name = image.split("/")[-1]
    # Strip tag (e.g., ":latest")
    name = name.split(":")[0]
    return name
...
class PyAirbyteBackend:
    ...
    ----------
    connector_name:
        Airbyte connector name, e.g. ``"source-jira"``.  Derived from the
        Docker image name via :func:`image_to_connector_name`.
    config:
    ...
    def _get_source(self):  # noqa: ANN202
        """Initialize the PyAirbyte source (lazy, cached)."""
    ...
    def discover_streams(self) -> list[str]:
        """Return available stream names from the connector."""
    ...
    def pull_stream(
        ...
        """Pull records from a single stream, with optional limit.

        Returns a list of plain dicts (Airbyte metadata stripped).
        """
        source = self._get_source()
        ...
        for record in source.get_records(stream, limit=limit):
            # Convert StreamRecord → plain dict, strip airbyte internal columns
            plain = {
    ...
    def pull_streams(
        ...
        """Pull multiple streams, applying per-stream or global limits.

        Parameters
        ----------
        streams:
            List of stream names to pull.
        per_stream_limits:
            Optional mapping of stream name → max records for that stream.
        global_limit:
            Fallback limit applied to streams not in ``per_stream_limits``.
