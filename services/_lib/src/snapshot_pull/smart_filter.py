...
class SeedStreamConfig:
    ...
    def from_dict(cls, data: dict[str, Any]) -> "SeedStreamConfig":
        ...
        return cls(stream=data["stream"], limit=data.get("limit"), follow=follow)
...
class SeedingConfig:
    ...
    def from_dict(cls, data: dict[str, Any]) -> "SeedingConfig":
        ...
        return cls(seed_streams=seed_streams, default_limit=data.get("default_limit"))
...
def apply_relational_filter(
    ...
    3. Child streams can have their own follow rules.
    ...
    while queue:
        ...
        if parent_keys is not None and follow_rule is not None:
            filtered = _filter_by_foreign_key(
                raw, follow_rule.foreign_key, parent_keys,
...
def _collect_keys(records: list[dict[str, Any]], key_field: str) -> set[Any]:
    keys: set[Any] = set()
