...
def test_delete_section(todoist_client: TodoistAPI):
    ...
    with pytest.raises(Exception):
        todoist_client.get_section(section_id=section_id)
...
def test_delete_section_removes_tasks(todoist_client: TodoistAPI):
    ...
    with pytest.raises(Exception):
        todoist_client.get_task(task_id=task1.id)

    with pytest.raises(Exception):
        todoist_client.get_task(task_id=task2.id)
...
def test_section_in_nonexistent_project(todoist_client: TodoistAPI):
    """Test that creating a section in a non-existent project still works (fake behavior)"""
    # The fake service may accept any project_id
    section = todoist_client.add_section(
        name="Orphan Section", project_id="nonexistent-project-id"
    )

    assert section is not None
    assert section.name == "Orphan Section"
    assert section.project_id == "nonexistent-project-id"
...
def test_delete_project_removes_sections(todoist_client: TodoistAPI):
    ...
    with pytest.raises(Exception):
        todoist_client.get_section(section_id=section1_id)

    with pytest.raises(Exception):
        todoist_client.get_section(section_id=section2_id)
