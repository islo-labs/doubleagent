...
def test_filter_tasks_by_section(todoist_client: TodoistAPI):
    ...
    # Note: Section creation requires direct API calls since SDK might not have it
    # For this test, we'll create tasks with section_id directly
    # In a real scenario, you'd first create sections via the API

    # Create tasks with section IDs
...
def test_filter_tasks_by_priority(todoist_client: TodoistAPI):
    ...
    # Filter by priority 4 (urgent) using filter_tasks with query "p4"
    urgent_tasks = []
    for page in todoist_client.filter_tasks(query="p4"):
        urgent_tasks.extend(page)

    # Verify only P4 task appears
...
def test_filter_high_priority_tasks(todoist_client: TodoistAPI):
    """Test filtering for high priority tasks (P3 and P4)"""
    ...
    # Filter P3 tasks using filter_tasks
    p3_tasks = []
    for page in todoist_client.filter_tasks(query="p3"):
        p3_tasks.extend(page)

    # Filter P4 tasks using filter_tasks
    p4_tasks = []
    for page in todoist_client.filter_tasks(query="p4"):
        p4_tasks.extend(page)

    # Verify correct filtering
    assert len(p3_tasks) == 1
    assert p3_tasks[0].id == task_p3.id
...
def test_combined_filters_project_and_priority(todoist_client: TodoistAPI):
    ...
    # Get all P4 tasks and filter by project client-side
    # (Real API would support combined filters like "#Project & p4")
    all_p4_tasks = []
    for page in todoist_client.filter_tasks(query="p4"):
        all_p4_tasks.extend(page)

    # Filter to only include tasks in the specific project
    project_urgent_tasks = [t for t in all_p4_tasks if t.project_id == project.id]
...
def test_close_task_with_recurring_due_date(todoist_client: TodoistAPI):
    """Test that closing a task with recurring due date works correctly"""
    ...
    # Note: In the real API, recurring tasks would be rescheduled
    # to their next occurrence. Our fake just marks them completed.


def test_filter_tasks_with_no_matches(todoist_client: TodoistAPI):
    """Test filtering with criteria that match no tasks"""
    # Create a task
    todoist_client.add_task(content="Test task", priority=1)

    # Filter by non-existent project
    tasks = []
    for page in todoist_client.get_tasks(project_id="nonexistent"):
        tasks.extend(page)
    assert len(tasks) == 0

    # Filter by non-existent label
    tasks = []
    for page in todoist_client.get_tasks(label="nonexistent"):
        tasks.extend(page)
    assert len(tasks) == 0

    # Filter by priority that doesn't exist
    tasks = []
    for page in todoist_client.filter_tasks(query="p4"):
        tasks.extend(page)
    assert len(tasks) == 0
...
def test_close_parent_task_does_not_close_subtasks(todoist_client: TodoistAPI):
    """Test that closing a parent task doesn't automatically close subtasks"""
    # Create parent and subtasks
    parent = todoist_client.add_task(content="Parent task")
    subtask1 = todoist_client.add_task(content="Subtask 1", parent_id=parent.id)
    subtask2 = todoist_client.add_task(content="Subtask 2", parent_id=parent.id)
    ...
    # Verify parent is completed but subtasks are still active
    parent_completed = todoist_client.get_task(task_id=parent.id)
    assert parent_completed.is_completed is True

    subtask1_active = todoist_client.get_task(task_id=subtask1.id)
    assert subtask1_active.is_completed is False

    subtask2_active = todoist_client.get_task(task_id=subtask2.id)
    assert subtask2_active.is_completed is False

    # Verify subtasks still appear in active list
    all_tasks = []
    for page in todoist_client.get_tasks():
        all_tasks.extend(page)

    task_ids = [t.id for t in all_tasks]
    assert parent.id not in task_ids  # Parent is completed
    assert subtask1.id in task_ids
    assert subtask2.id in task_ids
