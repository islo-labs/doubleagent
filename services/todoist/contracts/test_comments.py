...
def test_add_comment_with_attachment(todoist_client: TodoistAPI):
    """Test creating a comment with attachment param — real API returns 500 for SDK's attachment format"""
    ...
    # Real API ignores attachment parameter in REST comments
    ...
    assert retrieved_comment.attachment is None
...
def test_delete_comment(todoist_client: TodoistAPI):
    ...
    # Verify comment is deleted (no longer appears in task's comments list)
    comments_list = []
    for batch in todoist_client.get_comments(task_id=task.id):
        comments_list.extend(batch)
    assert comment.id not in [c.id for c in comments_list]
...
def test_update_comment_preserves_attachment(todoist_client: TodoistAPI):
    """Test that updating a comment works even when attachment param was passed at creation"""
    ...
    # Create comment with attachment (real API ignores attachment param)
    comment = todoist_client.add_comment(
