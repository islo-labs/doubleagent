...
def test_create_task_with_labels(todoist_client: TodoistAPI):
    ...
    assert task is not None
    assert sorted(task.labels) == sorted(["work", "urgent", "documentation"])

    # Verify round-trip: read back to prove persistence
    retrieved_task = todoist_client.get_task(task_id=task.id)
    assert sorted(retrieved_task.labels) == sorted(["work", "urgent", "documentation"])
...
def test_update_task_labels(todoist_client: TodoistAPI):
    ...
    assert updated_task.id == task.id
    assert sorted(updated_task.labels) == sorted(["new-label", "another-label"])

    # Verify round-trip: read back to prove persistence
    retrieved_task = todoist_client.get_task(task_id=task.id)
    assert sorted(retrieved_task.labels) == sorted(["new-label", "another-label"])
...
def test_delete_task(todoist_client: TodoistAPI):
    ...
    # Verify it's gone from active task list
    all_tasks = []
    for page in todoist_client.get_tasks():
        all_tasks.extend(page)
    assert task_id not in [t.id for t in all_tasks]
...
def test_complete_task_lifecycle(todoist_client: TodoistAPI):
    ...
    assert task.project_id == project.id
    assert sorted(task.labels) == sorted(["test", "lifecycle"])
    ...
    assert sorted(updated.labels) == sorted(["updated"])
